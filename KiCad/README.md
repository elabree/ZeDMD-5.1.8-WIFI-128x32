...
- 5V terminal block with 2 different use cases:
  - Output for the RGB panels, using +5V power from USB-C
  - 5V input for the DevKit, in case the RGB panels are on an external supply (jumper on J6)
...
- Two sockets (J1 and J3) for MAX98357A amplifier modules (Adafruit i2s class-d mono-amp). The modules are on the +5V supply (Devkit USB-C 5V, USB-C breakout Vin1 and Vin2).
- Two sockets (J8 and J5) for USB-C breakout boards to serve as +5V inputs. VBUS1 and VBUS2 drive the +5V supply via an "ideal diode" LM66200 module at J13. 
...
### MAX98357A modules
The amplifier modules use 5V supply voltage. The maximum required current depends on impedance of the connected speakers. If there is insufficient power avalable, the supply may dip during loud sounds causing clipping at the output. This may damage the speakers.
...
1. If USB-C is the preferred way to power everything: Then it is best to use a USB-C breakout (J5 or J8) to power the RGB matrix panel via J7; Do not place a jumper on J6.
2. The best alternative is a 5V/5A (25W) Meanwell switchmode power supply (SMPS). Connect the RGB matrix panels directly to the SMPS. Options to power the PCB including Devkit:
- USB-C (Devkit OR Breakout at J5 and/or J8); A 25W SMPS is needed.
- SMPS (Place jumper on J6 and connect the SMPS +5Vout to J7; Use a 40W SMPS.
*Note that option 2 allows full brighness of the RGB panels*
...
### 5Vin at J7
As explained a jumper (or wire bridge) at J6 is only needed in absence of USB-C power. In that case J7 becomes the 5V input.
...
3. Simplified power supply selection. Additional USB-C +5V input. Added ADXL345 Accelerometer: [production_Rev1.3.zip](/KiCad/ZeDMD_WiFi/production_Rev1.3.zip)
